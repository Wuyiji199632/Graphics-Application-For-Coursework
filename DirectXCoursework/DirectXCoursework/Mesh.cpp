...
Mesh::Mesh(const wchar_t* full_path):Resource(full_path)
{
	...
	std::string mtlDir = inputFile.substr(0, inputFile.find_last_of("\\/")); // material files path
   ...
   for (size_t m = 0; m < materials.size(); m++) {
	   ...
	   m_material_slots[m].material_id = index_global_offset;
   ...
   GraphicsEngine::get()->getVertexMeshLayoutShaderByCodeAndSize(&shader_byte_code, &size_shader);
   m_vertex_buffer=GraphicsEngine::get()->getRenderSystem()->createVertexBuffer(&list_vertices[0], sizeof(VertexMesh),list_vertices.size(),shader_byte_code,size_shader);
